...
/// A widget that constructs a list of night out category products
class NightOutList extends StatelessWidget {
  final List nightOutList = []; // Creating an empty list for night out products

  NightOutList({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // Iterate through the product list and add products that belong to the 'night out' category
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          // Constructing an 'ItemCard' for each product in the 'nightOutList'
          return ItemCard(
            product: nightOutList[index],
            press: () => Get.to(
              DetailsScreenwomen(
                product: nightOutList[
