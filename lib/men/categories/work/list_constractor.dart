...
/// A widget that constructs a list of work category products for men
class WorkListMan extends StatelessWidget {
  final List workListMan = []; // Creating an empty list for work products

  WorkListMan({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // Iterate through the men's product list and add products that belong to the 'work' category
    for (int i = 0; i < productsMan.length; i++) {
      if (productsMan[i].categorie == 'work') {
        workListMan.add(productsMan[i]);
      }
    }
    ...
    return Padding(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          // Constructing an 'ItemCard' for each product in the 'workListMan'
          return ItemCard(
            product: workListMan[index],
            press: () => Get.to(
              DetailsScreenman(
                product: workListMan[
